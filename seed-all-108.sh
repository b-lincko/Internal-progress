#!/bin/bash
# Complete Database Seed for CMMC Tracker - ALL 108 CONTROLS
# Run this on the SERVER

echo "=========================================="
echo " CMMC Tracker - Complete Database Seed"
echo "=========================================="
echo ""
...
# Create seed SQL
cat > /tmp/complete-seed.sql << 'SEEDSQL'
...
VALUES (gen_random_uuid(), 'CMMC Compliance Project', 'Initial compliance assessment and remediation', 'Active', 'High', 25, (SELECT id FROM users WHERE email='admin@local'), NOW(), NOW())
ON CONFLICT DO NOTHING;
SEEDSQL

echo "[2/3] Seeding database with ALL 108 controls..."
...
echo "Assets:     $(sudo docker exec cmmc-db psql -U cmmc -d cmmc2 -t -c "SELECT COUNT(*) FROM assets;" | xargs)"
echo "Projects:   $(sudo docker exec cmmc-db psql -U cmmc -d cmmc2 -t -c "SELECT COUNT(*) FROM projects;" | xargs)"
echo ""
